...
static void ntp_result(NTP_T* state, int status, time_t* result) {
    if(status == 0 && result) {
        ...
        utc->tm_mon + 1, utc->tm_year + 1900, utc->tm_hour, utc->tm_min, utc->tm_sec);
    }
...
// NTP data received
static void
ntp_recv(void* arg, struct udp_pcb* pcb, struct pbuf* p, const ip_addr_t* addr, u16_t port) {
    NTP_T* state = (NTP_T*)arg;
    ...
    p->tot_len == NTP_MSG_LEN && mode == 0x4 && stratum != 0) {
        ...
        seconds_buf[1] << 16 | seconds_buf[2] << 8 | seconds_buf[3];
...
void run_ntp_test(void) {
    ...
    while(true) {
        ...
        !state->dns_request_sent) {
            ...
            cyw43_arch_lwip_begin();
            int err = dns_gethostbyname(
            NTP_SERVER, &state->ntp_server_address, ntp_dns_found, state);
            cyw43_arch_lwip_end();
...
int main() {
    ...
    if(cyw43_arch_wifi_connect_timeout_ms("Fibertel WiFi528 2.4GHz",
       "Jm_23242526", CYW43_AUTH_WPA2_AES_PSK, 10000)) {
